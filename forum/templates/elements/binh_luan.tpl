          ...
          <div class="postbody span9">
            <div class="row-fluid">    
              <ul class="inline pull-right visible-phone">
                ...
                <li><a class="btn btn-small" href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/bao_cao/them?ma={$binh_luan_cha.ma}&loai=1" title="Báo cáo sai phạm" data-original-title="Báo cáo bình luận"><i class="icon-exclamation-sign"></i></a></li>
                <li><a class="btn btn-small" href="javascript:void(0);" title="" data-original-title="Trả lời bài viết" title="Trả lời bài viết"><i class="icon-comment" id="target-shown"></i></a></li>
              </ul>
              <div class="pull-left">
            ...
            <ul class="inline pull-right hidden-phone">
                ...
                <li><a class="btn btn-mini" href="../binh_luan/cap_nhat_binh_luan_dung?ma={$binh_luan_cha.ma}" title="Bình luận đúng" data-original-title="Bình luận đúng" style="color:green" ><i class="icon-ok"></i></a></li>
                  {/if}
                   {if $login != '' && $thanh_vien != '' && $thanh_vien.loai_thanh_vien != 3}
                  <li><span class="so_luong_thich_binh_luan{$binh_luan_cha.ma}">{$binh_luan_cha.thich}</span> <a  title="Thích" data-original-title="Thích" style="color:crimson" class="like-binh-luan{$binh_luan_cha.ma} btn btn-mini" title="Thích"><i class="icon-thumbs-up-alt thich-binh-luan{$binh_luan_cha.ma}" {if trang_thai_thich_binh_luan($binh_luan_cha.ma,$ma_nguoi_dung) == NULL } style="color:black;font-size:1.2em;cursor:pointer"{else}style="color:crimson;font-size:1.2em;cursor:pointer" {/if}></i></a> <li>
                   {if $ma_nguoi_dung != $binh_luan_cha.ma_nguoi_dung} <li>
                    
                    <a class="btn btn-mini" href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/bao_cao/them?ma={$binh_luan_cha.ma}&loai=1" title="{if trang_thai_bao_cao_binh_luan($binh_luan_cha.ma,$ma_nguoi_dung,$ma_dien_dan)==NULL}Báo cáo bình luận{else if}Đã báo cáo bình luận vi phạm{/if}" data-original-title=""{if trang_thai_bao_cao_binh_luan($binh_luan_cha.ma,$ma_nguoi_dung,$ma_dien_dan)!=NULL} onclick ="return false" {/if}><i class="icon-exclamation-sign" {if trang_thai_bao_cao_binh_luan($binh_luan_cha.ma,$ma_nguoi_dung,$ma_dien_dan)==NULL} style="color:black"{else}style="color:crimson"  {/if} ></i></a>
                    </li>{/if}
                    ...
                    <li><a class="btn btn-mini" data-toggle="collapse" data-target="#{$binh_luan_cha.ma}{$binh_luan_cha.ma_nguoi_dung}" data-original-title="Trả lời bài viết" title="Trả lời bài viết"><i class="icon-comment"></i></a></li>
                  
                     {else}
                        	<li><span class="so_luong_thich_binh_luan{$binh_luan_cha.ma}">{$binh_luan_cha.thich}</span> <a class="btn btn-mini" data- data-original-title="Lượt thích" onclick="return false" title="Thích"><i class="icon-thumbs-up-alt thich-binh-luan{$binh_luan_cha.ma}" style="color:black;font-size:1.2em;"></i></a></li>
                      {/if}
                     <li><a class="btn btn-mini" data-toggle="collapse" data-target="#{$binh_luan_cha.ma}" data-original-title="Xem các trả lời" title="Xem các trả lời"><i class="icon-comment"> <span class="badge badge-info">
	  ...
	  if($.trim(arr[0]) == 'dislike'){
        ...
        <div  class="well well-small" style="background-color: white;margin-bottom:0px;border-radius:0px" id="{$binh_luan_con.ma}{$binh_luan_con.ma_bai_viet}">
          ...
          <div class="postbody span9">
                <div class="row-fluid">
                  <ul class="inline pull-right visible-phone">
                    <li><a class="btn btn-small" href="../bao_cao/index.php" title="" data-original-title="Báo cáo sai phạm" title="Báo cáo sai phạm"><i class="icon-exclamation-sign"></i></a></li>
                ...
                <ul class="inline pull-right hidden-phone">
                   {if $login!='' && $login.ma == $bai_viet.ma_nguoi_dang}
                <li><a class="btn btn-mini" href="../binh_luan/cap_nhat_binh_luan_dung?ma={$binh_luan_con.ma}" title="Bình luận đúng" data-original-title="Bình luận đúng" style="color:green" ><i class="icon-ok"></i></a></li>
                  {/if}
                   {if $login != '' && $thanh_vien != '' && $thanh_vien.loai_thanh_vien != 3}
                  <li><span class="so_luong_thich_binh_luan{$binh_luan_con.ma}">{$binh_luan_con.thich}</span> <a title="Thích" data-original-title="Thích" style="color:crimson" class="like-binh-luan{$binh_luan_con.ma} btn btn-mini"><i class="icon-thumbs-up-alt thich-binh-luan{$binh_luan_con.ma}" {if trang_thai_thich_binh_luan($binh_luan_con.ma,$ma_nguoi_dung) == NULL } style="color:black;font-size:1.2em;cursor:pointer"{else}style="color:crimson;font-size:1.2em;cursor:pointer" {/if}></i></a> <li>
               {if $ma_nguoi_dung != $binh_luan_con.ma_nguoi_dung} 
                    <li><a class="btn btn-mini" href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/bao_cao/them?ma={$binh_luan_con.ma}&loai=1" title="{if trang_thai_bao_cao_binh_luan($binh_luan_con.ma,$ma_nguoi_dung,$ma_dien_dan)==NULL}Báo cáo sai phạm{else if}Đã báo cáo bình luận vi phạm{/if}" data-original-title="Báo cáo bình luận" {if trang_thai_bao_cao_binh_luan($binh_luan_con.ma,$ma_nguoi_dung,$ma_dien_dan)!=NULL} onclick ="return false" {/if}><i class="icon-exclamation-sign"  {if trang_thai_bao_cao_binh_luan($binh_luan_con.ma,$ma_nguoi_dung,$ma_dien_dan)==NULL} style="color:black"{else}style="color:crimson"  {/if} ></i></a></li{/if}
                     {if $ma_nguoi_dung == $binh_luan_con.ma_nguoi_dung}<li><a href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/binh_luan/cap_nhat?ma={$binh_luan_con.ma}" class="btn btn-mini" title="Chỉnh sửa bình luận"><i class="icon-pencil" style="color:black;font-size:1.2em;cursor:pointer"></i></a></li>{/if}
                   {else}
