...
<div class="content-box-content">
	...
	<div class="tab-content default-tab" id="tab1"> <!-- This is the target div. id must match the href of this div's tab -->
        ...
        <br /><br />
        {/if}													
		<form method="get" action="bai_viet.php" name="fSearch" id="fSearch">
          Từ Khóa
          <input type="text" name="tu_khoa" class="text-input small-input" value="">
          <input class="button" type="submit" value="Tìm Kiếm">
          <input class="button" type="button" value="Tất cả" onclick="window.location.href='bai_viet.php'">
        </form>
...
<thead>
		  ...
		  <tfoot>
			  <tr>
                ...
                <img src="/forum/admin/templates/images/arrow_ltr.png" /> <input name="xoa_muc_chon" class="button" type="submit" value="Xóa Các Mục Đã Chọn" onclick="return confirm('Bạn có chắc chắn muốn xóa không ?')"></div>
                {/if}
...
<tbody>
            ...
            {foreach $ds_bao_cao_bai_viet as $k=>$bao_cao}   
               ...
               {if $thanh_vien.loai_thanh_vien==0} <td><input name="data[]" type="checkbox" value="{post_encode($bao_cao.ma)}"></td>{/if}
         ...
         {if $thanh_vien.loai_thanh_vien==0 || tim_kiem_chuoi($qcm, $bao_cao.ma_loai_chuyen_muc)!==false}         
            {if $bao_cao.trang_thai==1}      
                  <a href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/admin/bao_cao/xoa.php?ma={url_encode($bao_cao.ma)}" onclick="return confirm('Bạn có chắc chắn muốn xóa báo cáo vi phạm này không ?')" title="xóa"><img src="/forum/admin/templates/images/icons/cross.png" alt="Delete"></a>
			{/if}
         {/if}   

			{if $thanh_vien.loai_thanh_vien==0 || tim_kiem_chuoi($qcm, $bao_cao.ma_loai_chuyen_muc)!==false}	
                {if $bao_cao.trang_thai==0}
                   <a href="/{$dien_dan.ma_linh_vuc}/{$dien_dan.domain}/admin/bao_cao/xu_ly_bao_cao_bai_viet.php?ma_bao_cao={url_encode($bao_cao.ma)}" title="Xử lý vi phạm"><img src="/forum/admin/templates/images/warning-icon.png" alt="Xử lý báo cáo vi phạm" width="16"></a>
                {/if}
            {/if}       
                </td>
              </tr>
