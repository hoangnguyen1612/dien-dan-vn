...
formtowizard.prototype={
	...
	loadsection:function(rawi, bypasshooks){
  	...
  	//doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			this.paginatediv.$status.text(""+(i+1)+" / "+this.sections.count) //update current page status text
	...
	validate:function(section){
		...
		var validated=true, invalidtext=["Vui lòng điền đầy đủ thông tin vào các ô:\n"]
		...
		if(ten_dien_dan!='')
		{
			...
			jQuery.ajax({
				type:"POST",
				url:"kiem_tra.php",
				data: {'ten_dien_dan':ten_dien_dan, 'ma_linh_vuc':ma_linh_vuc},
		...
		if(!check)
			alert('Diễn đàn thuộc lĩnh vực bạn chọn đã tồn tại, vui lòng chọn tên diễn đàn khác');
		return check;	
	},
	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			if (setting.revealfx[0]=="slide"){ //create outer DIV that will house all the fieldset.sectionwrap elements
				$sectionswrapper=$('<div style="position:relative;  max-height:600px; width:700px;"></div>').insertBefore($sections.eq(0)) //add DIV above the first fieldset.sectionwrap element
				$sectionswrapper_inner=$('<div style="position:absolute;left:0;top:0; width:700px;"></div>') //create inner DIV of $sectionswrapper that will scroll to reveal a fieldset element
			}
			...
			$sections.each(function(i){ //loop through $sections again
				...
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Bước '+(i+1)+'<div class="smalltext">'+$section.find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
			...
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">Quay lại</span> <span class="status">Step 1 of </span> <span class="next" style="float:right">Kế tiếp</span></div>')
